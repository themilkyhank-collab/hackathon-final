...
app = FastAPI(title="Aesteel Engine Diagnostics API", version="2.3.0")
    ...
    if _pipeline is None:
        try:
            if MODEL_PATH.exists():
                ...
                # Rebuild artifacts produced before anomaly/explainability support.
                if (getattr(loaded, "selected_features", None) and getattr(loaded, "model", None) is not None
...
@app.get("/api/health")
def health() -> dict:
    return {"status": "ok", "api": "online", "model": "online" if MODEL_PATH.exists() else "ready-on-startup", "anomaly_detector": "IsolationForest"}


@app.get("/api/model")
def model_info() -> dict:
    pipeline = get_pipeline()
    return {"model": "XGBoost", "anomaly_detector": "IsolationForest", "training_data": "train.csv only",
            "reference_data": "val.csv for semantic pseudo-labeling only", "features": len(pipeline.selected_features),
            "features_before_selection": len(pipeline.feature_names), "selected_features": pipeline.selected_features,
            "hyperparameters": pipeline.params, "training_metrics": pipeline.training_metrics,
...
def feature_analysis() -> dict:
    pipeline = get_pipeline()
    return {"features_before_selection": len(pipeline.feature_names), "features_after_selection": len(pipeline.selected_features),
            "selected_features": pipeline.selected_features, "importance": pipeline.feature_analysis}


def _validate_df(df: pd.DataFrame) -> pd.DataFrame:
    required = ["engine_id", "cylinder", *FREQ_COLS]
    missing = [c for c in required if c not in df.columns]
    if missing: raise HTTPException(status_code=422, detail=f"Missing required columns: {missing}")
    cleaned = clean_spectrum(df)
    valid, message = validate_spectrum(cleaned)
    if not valid: raise HTTPException(status_code=422, detail=message)
    return cleaned
...
async def diagnose(file: UploadFile = File(...)) -> JSONResponse:
    ...
    try:
        ...
        if len(payload) > 20 * 1024 * 1024: raise HTTPException(status_code=413, detail="File is too large (max 20 MB)")
        ...
        return JSONResponse({"status": "ok", "filename": file.filename, "row_count": len(rows), "analysis_time_ms": round(elapsed_ms, 1),
            "rows": rows,
            "summary": {"label_counts": label_counts, "severity_counts": severity_counts,
